...
(function(){
  ...
  var L=[{n:190,s:0.0022,sh:5,a:0.42,r0:0.35,r1:0.9},
         {n:110,s:0.0060,sh:13,a:0.62,r0:0.6, r1:1.35},
         {n:46, s:0.0125,sh:26,a:0.9, r0:0.9, r1:1.9}];
  ...
  function sat(now){
    ...
    var p=((now+6000)%34000)/34000; if(p>0.42) return;
    var k=p/0.42, x=-40+k*(W+80), y=H*0.18+Math.sin(k*Math.PI)*-H*0.045+k*H*0.06, f=Math.sin(k*Math.PI);
    ctx.save(); ctx.globalCompositeOperation='lighter';
    var g=ctx.createLinearGradient(x-130,y,x,y);
    g.addColorStop(0,'rgba(232,228,216,0)'); g.addColorStop(1,'rgba(232,228,216,'+(0.20*f)+')');
    ctx.strokeStyle=g; ctx.lineWidth=1.1;
    ctx.beginPath(); ctx.moveTo(x-130,y+5); ctx.lineTo(x,y); ctx.stroke();
    ctx.fillStyle='rgba(245,229,167,'+(0.95*f)+')';
    ctx.beginPath(); ctx.arc(x,y,1.7,0,6.283); ctx.fill();
    ctx.fillStyle='rgba(245,229,167,'+(0.16*f)+')';
    ctx.beginPath(); ctx.arc(x,y,6.5,0,6.283); ctx.fill();
    ctx.restore();
  }
  function frame(now){
    if(W&&H){
