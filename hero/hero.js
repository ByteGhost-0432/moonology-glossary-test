...
(function(){
  ...
  var sv = root.querySelector('.mng-sat');
  ...
  function resize(){
    ...
    ctx.setTransform(DPR,0,0,DPR,0,0); sx.setTransform(DPR,0,0,DPR,0,0); build();
  }
  function sat(now){
    ...
    var g=sx.createLinearGradient(x-230,y,x,y);
    g.addColorStop(0,'rgba(232,228,216,0)'); g.addColorStop(1,'rgba(232,228,216,'+(0.72*f)+')');
    sx.strokeStyle=g; sx.lineWidth=2.1;
    sx.beginPath(); sx.moveTo(x-230,y+8); sx.lineTo(x,y); sx.stroke();
    sx.fillStyle='rgba(245,229,167,'+(0.95*f)+')';
    sx.beginPath(); sx.arc(x,y,3.1,0,6.283); sx.fill();
    sx.fillStyle='rgba(245,229,167,'+(0.38*f)+')';
    sx.beginPath(); sx.arc(x,y,15,0,6.283); sx.fill();
    sx.restore();
  }
  function frame(now){
    if(W&&H){
