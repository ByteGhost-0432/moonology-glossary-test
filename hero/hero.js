/* Moonology rotating hero — lives here, not in WordPress. */
(function(){
  ...
  mount.querySelector('.mng-h1').textContent = TAGLINE;   // textContent: WordPress text is never HTML
  (function(){
  ...
  fetch('https://test.moonologyartgallery.com/hero/current.json', { cache:'no-cache' })
    ...
    .then(function(d){
      if (!d) return;
      // 2026-09-24: the hero is now the channel BANNER. Name/spec/link/cta moved to the
      // channel page (latest upload, from the live feed), so only the image rotates here.
  ...
  function sat(now){
    // Deliberately drawn on the STARFIELD canvas, behind the moon image (z0 vs z1).
    // Lawrence, 2026-08-04: "It looked better flying behind the moon." An overlay version
    // that transited the disc was tried and rejected — the occlusion is what sells the
    // depth. It emerges left, crosses open sky, and vanishes behind the limb.
    // One crossing every ~34s. Rare enough to read as a sighting, not a screensaver.
    var p=((now+4000)%21000)/21000; if(p>0.46) return;   // cleared above
...
/* Channel page — the YouTube channel mirrored under the hero. Loaded from here so the
   homepage needs no WordPress edit. See channel/channel.js. */
(function(){
  if (document.getElementById('mng-channel-js')) return;
  var s = document.createElement('script');
  s.id = 'mng-channel-js'; s.async = true;
  s.src = (window.MNG_CHANNEL_BASE || 'https://test.moonologyartgallery.com/channel/') + 'channel.js';
